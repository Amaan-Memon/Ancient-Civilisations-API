...
const populateCivsTable = async() =>{
    for(let i = 0; i < ancientCivs.length; i++){
        const result = await query(
            ...
            [ancientCivs[i].name, ancientCivs[i].knownFor, ancientCivs[i].duration, ancientCivs[i].size, ancientCivs[i].capital, ancientCivs[i].famousRulers, ancientCivs[i].religion, ancientCivs[i].language]
        );
        console.log(`${ancientCivs[i].name} added to DB`);
    }
}
