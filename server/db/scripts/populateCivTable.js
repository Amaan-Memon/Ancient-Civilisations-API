...
const populateCivsTable = async () => {
  for (let i = 0; i < ancientCivs.length; i++) {
    const result = await query(
      `INSERT INTO civs(name, knownFor, duration, size, capital, famousRulers, religion, language ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [
        ancientCivs[i].name,
    ...
    console.log(
      `${ancientCivs[i].name} added to DB and this is the result: ${result}`
    );
  }
};
